...
- `apps/web` Vitest checks deny valid-looking unauthenticated requests before any catalog mutation, inventory change, order status/tracking update, customer-discount update, media association, integration retry or CSV order export. The export route also returns `403` to an authenticated non-admin and, on its admin path, emits a private CSV that neutralizes spreadsheet formulas.
