...
| AUD-013 | P1 — external access required | RB-010 remote rehearsal cannot run from the current workspace: Cloudflare reports `loggedIn: false`, Vercel reports `Logged out`, and no linked Vercel project metadata is present. | Authenticate the approved Cloudflare/Vercel accounts and identify the staging project, non-production R2 bucket, domain owner and rollback deployments. Then execute `docs/CUTOVER_RUNBOOK.md`; do not switch production before all gates pass. | 6 |
